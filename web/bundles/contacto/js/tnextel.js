(function($){if(typeof $.fn.btnAddRowtn== "undefined"){
	...
	ExpandableTable.prototype.live=function(){
		if (!this.goLive){
			var t=this;
			this.update();
			$(".addRowTNextel"+this.seed)
			.live("click",function(){
				var newRow=t.addRow();
			});
			$(".delRowTNextel"+this.seed)
			.live("click",function(){
	...
	ExpandableTable.prototype.updateRowCount=function(){
		if(this.displayRowCountToTN){
			var count=$("."+this.cloneClass,this.target).size();
			$("."+this.displayRowCountToTN,this.target).each(function(i,v){
	...
	ExpandableTable.prototype.update=function(){
		var t=this;
		this.delRowButtons=$(".delRowTNextel"+this.seed,this.target);
	...
	ExpandableTable.prototype.addRow=function(){
		...
		if(!this.maxRow || (this.maxRow && $("."+this.cloneClass).size()<this.maxRow)){
			...
			if(this.maxRow && $("."+this.cloneClass).size()>=this.maxRow) 
				$(".addRowTNextel"+this.seed).attr("disabled",true);
	...
	$.fn.btnAddRowtn=$.fn.tableAutoAddRow=function(options,func){
		var callBack;
		...
		this.each(function(){
			...
			if(options.rowCountAttr && typeof $(this).attr(options.rowCountAttr)!="undefined")
				options.displayRowCountToTN=$(this).attr(options.rowCountAttr);
			...
			if(tbl.size()>0){
				if(typeof tbl.data(className)==="undefined" || tbl.data(className)===null){
					...
					if(this.nodeName.toLowerCase()!="table")
						$(this)
							.addClass("addRowTNextel"+etbl.seed)
							.data(className,etbl);
				}else{
					if(this.nodeName.toLowerCase()!="table")
						$(this)
							.addClass("addRowTNextel"+tbl.data(className).seed)
				...
				etbl.displayRowCountToTN=options.displayRowCountToTN;
	...
	$.fn.btnDelRowtn=function(options,func){
		var callBack;
		if (typeof options=="object")
			callBack=(func && $.isFunction(func)) ? func :null; 
		else
		...
		this.each(function(){
			...
			if(tbl.size()>0){
				if(typeof tbl.data(className)==="undefined" || tbl.data(className)===null){
					...
					$(this)
						.addClass("delRowTNextel"+etbl.seed)
						.data(className,etbl);
				}else{
					$(this)
						.addClass("delRowTNextel"+tbl.data(className).seed)
						.data(className,tbl.data(className));
				}
				if($(this).data(className)) {
					etbl=$(this).data(className);

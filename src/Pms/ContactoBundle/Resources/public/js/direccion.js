(function($){if(typeof $.fn.btnAddRowDireccion== "undefined"){
	...
	ExpandableTable.prototype.live=function(){
		if (!this.goLive){
			var t=this;
			this.update();
			$(".addRowDireccion"+this.seed)
			.live("click",function(){
				var newRow=t.addRow();
			});
			$(".delRowDireccion"+this.seed)
			.live("click",function(){
	...
	ExpandableTable.prototype.updateRowCount=function(){
		if(this.displayRowCountToDireccion){
			var count=$("."+this.cloneClass,this.target).size();
			$("."+this.displayRowCountToDireccion,this.target).each(function(i,v){
	...
	ExpandableTable.prototype.update=function(){
		var t=this;
		this.delRowButtons=$(".delRowDireccion"+this.seed,this.target);
	...
	ExpandableTable.prototype.addRow=function(){
		...
		if(!this.maxRow || (this.maxRow && $("."+this.cloneClass).size()<this.maxRow)){
			...
			if(this.maxRow && $("."+this.cloneClass).size()>=this.maxRow) 
				$(".addRowDireccion"+this.seed).attr("disabled",true);
	...
	$.fn.btnAddRowDireccion=$.fn.tableAutoAddRow=function(options,func){
		var callBack;
		...
		options=$.extend({
			...
			displayRowCountToDireccion:true,
			maxRowAttr:null,
			hideFirstOnly:null,
			showFirstOnly:null,
			cloneClass:null,
		...
		this.each(function(){
			...
			if(options.rowCountAttr && typeof $(this).attr(options.rowCountAttr)!="undefined")
				options.displayRowCountToDireccion=$(this).attr(options.rowCountAttr);
			...
			if(tbl.size()>0){
				if(typeof tbl.data(className)==="undefined" || tbl.data(className)===null){
					...
					if(this.nodeName.toLowerCase()!="table")
						$(this)
							.addClass("addRowDireccion"+etbl.seed)
							.data(className,etbl);
				}else{
					if(this.nodeName.toLowerCase()!="table")
						$(this)
							.addClass("addRowDireccion"+tbl.data(className).seed)
				...
				etbl.displayRowCountToDireccion=options.displayRowCountToDireccion;
	...
	$.fn.btnDelRowDireccion=function(options,func){
		var callBack;
		if (typeof options=="object")
			callBack=(func && $.isFunction(func)) ? func :null; 
		else
		...
		this.each(function(){
			...
			if(tbl.size()>0){
				if(typeof tbl.data(className)==="undefined" || tbl.data(className)===null){
					...
					$(this)
						.addClass("delRowDireccion"+etbl.seed)
						.data(className,etbl);
				}else{
					$(this)
						.addClass("delRowDireccion"+tbl.data(className).seed)
						.data(className,tbl.data(className));
				}
				if($(this).data(className)) {
					etbl=$(this).data(className);

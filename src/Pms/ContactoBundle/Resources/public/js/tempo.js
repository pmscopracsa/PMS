(function($){if(typeof $.fn.btnAddRowx== "undefined"){
	...
	ExpandableTable.prototype.updateRowCount=function(){
		if(this.displayRowCountTox){
			var count=$("."+this.cloneClass,this.target).size();
			$("."+this.displayRowCountTox,this.target).each(function(i,v){
	...
	ExpandableTable.prototype.update=function(){
		var t=this;
		this.delRowButtons=$(".xdelRow"+this.seed,this.target);
	...
	ExpandableTable.prototype.addRow=function(){
		...
		if(!this.maxRow || (this.maxRow && $("."+this.cloneClass).size()<this.maxRow)){
			...
			if(this.maxRow && $("."+this.cloneClass).size()>=this.maxRow) 
				$(".xaddRow"+this.seed).attr("disabled",true);
	...
	$.fn.btnAddRowx=$.fn.tableAutoAddRow=function(options,func){
		var callBack;
		...
		this.each(function(){
			...
			if(options.rowCountAttr && typeof $(this).attr(options.rowCountAttr)!="undefined")
				options.displayRowCountTox=$(this).attr(options.rowCountAttr);
			...
			if(tbl.size()>0){
				if(typeof tbl.data(className)==="undefined" || tbl.data(className)===null){
					...
					if(this.nodeName.toLowerCase()!="table")
						$(this)
							.addClass("xaddRow"+etbl.seed)
							.data(className,etbl);
				}else{
					if(this.nodeName.toLowerCase()!="table")
						$(this)
							.addClass("xaddRow"+tbl.data(className).seed)
				...
				etbl.displayRowCountTox=options.displayRowCountTox;
	...
	$.fn.btnDelRowx=function(options,func){
		var callBack;
		if (typeof options=="object")
			callBack=(func && $.isFunction(func)) ? func :null; 
		else
		...
		this.each(function(){
			...
			if(tbl.size()>0){
				if(typeof tbl.data(className)==="undefined" || tbl.data(className)===null){
					...
					$(this)
						.addClass("xdelRow"+etbl.seed)
						.data(className,etbl);
				}else{
					$(this)
						.addClass("xdelRow"+tbl.data(className).seed)
						.data(className,tbl.data(className));
				}
				if($(this).data(className)) {
					etbl=$(this).data(className);

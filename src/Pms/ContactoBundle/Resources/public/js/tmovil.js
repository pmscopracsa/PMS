(function($){if(typeof $.fn.btnAddRowtm== "undefined"){
	...
	ExpandableTable.prototype.live=function(){
		if (!this.goLive){
			var t=this;
			this.update();
			$(".addRowTMovil"+this.seed)
			.live("click",function(){
				var newRow=t.addRow();
			});
			$(".delRowTMovil"+this.seed)
			.live("click",function(){
	...
	ExpandableTable.prototype.updateRowCount=function(){
		if(this.displayRowCountToTM){
			var count=$("."+this.cloneClass,this.target).size();
			$("."+this.displayRowCountToTM,this.target).each(function(i,v){
	...
	ExpandableTable.prototype.update=function(){
		var t=this;
		this.delRowButtons=$(".delRowTMovil"+this.seed,this.target);
	...
	ExpandableTable.prototype.addRow=function(){
		...
		if(!this.maxRow || (this.maxRow && $("."+this.cloneClass).size()<this.maxRow)){
			...
			if(this.maxRow && $("."+this.cloneClass).size()>=this.maxRow) 
				$(".addRowTMovil"+this.seed).attr("disabled",true);
	...
	$.fn.btnAddRowtm=$.fn.tableAutoAddRow=function(options,func){
		var callBack;
		...
		this.each(function(){
			...
			if(options.rowCountAttr && typeof $(this).attr(options.rowCountAttr)!="undefined")
				options.displayRowCountToTM=$(this).attr(options.rowCountAttr);
			...
			if(tbl.size()>0){
				if(typeof tbl.data(className)==="undefined" || tbl.data(className)===null){
					...
					if(this.nodeName.toLowerCase()!="table")
						$(this)
							.addClass("addRowTMovil"+etbl.seed)
							.data(className,etbl);
				}else{
					if(this.nodeName.toLowerCase()!="table")
						$(this)
							.addClass("addRowTMovil"+tbl.data(className).seed)
				...
				etbl.displayRowCountToTM=options.displayRowCountToTM;
	...
	$.fn.btnDelRowtm=function(options,func){
		var callBack;
		if (typeof options=="object")
			callBack=(func && $.isFunction(func)) ? func :null; 
		else
		...
		this.each(function(){
			...
			if(tbl.size()>0){
				if(typeof tbl.data(className)==="undefined" || tbl.data(className)===null){
					...
					$(this)
						.addClass("delRowTMovil"+etbl.seed)
						.data(className,etbl);
				}else{
					$(this)
						.addClass("delRowTMovil"+tbl.data(className).seed)
						.data(className,tbl.data(className));
				}
				if($(this).data(className)) {
					etbl=$(this).data(className);
